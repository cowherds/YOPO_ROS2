...
class YopoNet:
    def __init__(self, config, weight):
        ...
        # ros subscriber
        self.odom_sub = self.ros.create_subscription(Odometry, self.config["odom_topic"], self.callback_odometry, queue_size=1)
        self.depth_sub = self.ros.create_subscription(Image, self.config["depth_topic"], self.callback_depth, queue_size=1)
        self.goal_sub = self.ros.create_subscription(PoseStamped, "/move_base_simple/goal", self.callback_set_goal, queue_size=1)
        # ros timer
        self.ros.sleep(1.0)  # wait connection...
...
def parser():
    ...
    parser.add_argument("--ros_version", type=str, default="auto", choices=["auto", "ros1", "ros2"],
                        help="ROS backend selection: auto|ros1|ros2")
    return parser
